...
// export function AddItemModal({ isOpen, onClose }) {
...
//       formData.append("userId", userId); // Replace with actual logged-in user ID
...
//           <select value={category} onChange={(e) => setCategory(e.target.value)} className="w-full border p-2 rounded-md" required>
//             <option value="" disabled>{loading ? "Loading categories..." : "Select Category"}</option>
//             {error ? <option value="" disabled>{error}</option> : categories.map((cat, index) => <option key={index} value={cat}>{cat}</option>)}
//           </select>
...
//                 <img key={index} src={URL.createObjectURL(image)} alt={`Preview ${index + 1}`} className="w-full h-20 object-cover rounded-md" />
//               ))}
//             </div>
//           )}
...
//           <DialogFooter>
//             <Button type="submit" className="bg-gray-800 text-white hover:bg-gray-700" disabled={uploading}>
//               {uploading ? "Adding..." : "Add Item"}
//             </Button>
//             <Button variant="ghost" className="text-gray-700 hover:bg-gray-200" onClick={onClose}>
//               Cancel
//             </Button>
//           </DialogFooter>
//         </form>
